...
async def create_relationship(loop) -> str:
    ...
    await invitation
    return rel_did, qr_string  # return owning DID for the connection
...
async def update_webhook_endpoint():
    ...
    # for local use ngrok:
    # webhook = "http://facd7454d6cb.ngrok.io/webhook"
    if not webhook:
...
async def api_issue_digital_identity(request):
    ...
    # Store this user_uuid against relationship_did, so that in /webhook endpoint
    # we can fetch user_uuid using relationship_did
...
def fn_dw_identity_status(request):
    user_uuid = request.match_info.get("uuid", "")
    status_code = dbhelper.get_user_status_by_uuid(user_uuid)
    response_data = json.dumps({"status": 200, 'user_status_code': status_code})
...
my_web_app.add_routes(
    [web.get('/', home),
     ...
     web.get('/api_dw_identity_status/{uuid}', fn_dw_identity_status),
     web.get('/api_issue_digital_identity/{uuid}', api_issue_digital_identity)]
)
